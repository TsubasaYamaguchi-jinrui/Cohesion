...
injury_fin %>% 
  ...
  mutate(rate_agg_tm = agg_tm*60/duration,
         rate_agg_ntm = agg_ntm*60/duration) -> agg_injury_ntm
```
...
- 説明変数: 群れオスからの攻撃頻度(`rate`_agg_tm`)、群れ外オスからの攻撃頻度(`rate_agg_ntm``)、メスの発情の有無(`rs2`)、攻撃頻度と発情の有無の交互作用、調査期間(`study_period`)  
- ランダム切片: メスID(`femaleID`)      

```{r}
m_agginj <- brm(injury01 ~ rate_agg_tm + rate_agg_ntm + rs2 + rate_agg_ntm:rs2 + rate_agg_tm:rs2 + study_period + (1|femaleID),
                family = bernoulli,
                iter = 5000, warmup = 2500, seed = 13,
                 prior = c(prior(student_t(4,0,10), class = "b"),
                           prior(student_t(4,0,10), class = "Intercept"),
...
**群れ外オスからの攻撃**は、発情メスと非発情メスいずれも有意な影響を持たなかった。  
```{r}
...
90%確信区間についてみると、非発情メスについては確信区間が0を含まなかった。  
```{r}
estimate_slopes(m_agginj,
                trend = "rate_agg_ntm",
                at = "rs2 = c(0,1)",
...
**群れオスからの攻撃**は、いずれの発情状態についても有意に影響していた。    
```{r}
estimate_slopes(m_agginj,
                trend = "rate_agg_tm",
                at = "rs2 = c(0,1)")
...
injury_fin %>% 
  ...
  ## 被攻撃頻度を算出  
  mutate(rate_agg = no_agg*60/duration) -> agg_injury_all
```
