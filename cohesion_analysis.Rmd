...
female_time %>% 
  ...
  filter(presence != "?" & presence != "DD" & presence != "NS" & presence != "NA") -> female_time_long

focal_raw_fin %>% 
  distinct(date, no_focal, subject) %>% 
  left_join(female_all %>% select(date, femaleID, presence)) -> a
```
