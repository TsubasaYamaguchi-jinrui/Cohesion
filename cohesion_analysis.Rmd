...
focal_raw_nm19_fin %>% 
  ...
  filter(RGsuc3 == "1") %>% 
  filter(hud != "1") %>% 
  mutate(prop_female = (1+RGfemale)/no_female) %>% 
  group_by(no_focal, subject, study_period, date, no_female) %>% 
  summarise(N = n(),
            sum = sum(RGfemale),
            mean = mean(prop_female)) %>% 
  ungroup() %>% 
  filter(N >= 15) -> focal_RGfemale_sum_nm
```
...
focal_raw_prox_fin %>% 
  ...
  filter(RGsuc3 == "1") %>% 
  filter(rs2 == "0") %>%
  mutate(prop_female = (RGfemale+1)/no_female) %>% 
  group_by(no_focal, subject, study_period, TY, IT, date, no_female) %>% 
  summarise(N = n(),
            sum = sum(RGfemale),
            mean = mean(prop_female)) %>% 
  ungroup() %>% 
  filter(N >= 15) -> focal_RGfemale_sum
```

データを結合する。  
```{r}
bind_rows(focal_RGfemale_sum, focal_RGfemale_sum_nm) %>% 
  replace_na(list(TY = 1, IT = 1)) %>% 
  mutate(date = as.factor(date)) %>% 
  mutate(season = ifelse(str_detect(study_period,"nm"),"nm","m")) %>% 
  mutate(bunbo = no_female*N) -> focal_RGfemale_sum_all
```

##### 分析  
調査期間ごとの比較を行う。  
```{r}
m_RGfemale_comp <- brm(data = focal_RGfemale_sum_all,
                       sum|trials(bunbo) ~ study_period + (1|subject) + (1|date) ,
                       ...
                       family = "beta_binomial",
                       backend = "cmdstanr",
                       file = "model/m_RGfemale_comp")
```

```{r, include = FALSE}
check_RGfemale_comp <- dh_check_brms(m_RGfemale_comp)
```


**結果の確認**  
```{r}
estimate_contrasts(m_RGfemale_comp,
                   contrast = "study_period")
```

```{r}
focal_RGfemale_sum_all %>% 
  filter(TY != "2") %>% 
  mutate(TY2 = ifelse(season == "m" & TY == "1", "mTY",
                      ifelse(season == "m" & TY == "0", "mnotTY", "nm"))) %>% 
  mutate(cat = str_c(study_period,"_",TY2)) %>% 
  mutate(bunbo = no_female*N) -> focalRGfemale_sum_TY

m_RGfemale_comp_TY <- brm(data = focalRGfemale_sum_TY,
                       sum|trials(bunbo) ~ cat + (1|subject) + (1|date) ,
                       iter = 5000, warmup = 2500, seed = 13,
                       prior = c(prior(student_t(4,0,10), class = "b"),
                           prior(student_t(4,0,10), class = "Intercept"),
                           prior(student_t(4,0,10), class = "sd")),
                       ...
                       family = "beta_binomial",
                       backend = "cmdstanr",
                       file = "model/m_RGfemale_comp_TY")
```

```{r}
check_RGfemale_comp <- dh_check_brms(m_RGfemale_comp_TY)
```

```{r}
estimate_contrasts(m_RGfemale_comp_TY,
                   contrast = "cat")
```
