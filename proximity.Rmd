```{r}
focal18_raw <- read_excel("C:/Users/Tsubasa Yamaguchi/Desktop/Study/data/2018mating/2018mating_raw.xlsx",
                      sheet = "focal_raw")

focal18_raw %>% 
  mutate(type = ifelse(str_detect(Event,"Bite"),4,
                                  ifelse(str_detect(Event,"Grab"),3,
                                         ifelse(str_detect(Event,"Slap"),3,
                                                    ifelse(str_detect(Event,"Ch"),2,
                                                                      ifelse(str_detect(Event,"Lg"),1,
                                                                             ifelse(str_detect(Event,"Th"),0,
                                                                                    ifelse(str_detect(Event,"Sup"),0,"NA")))))))) -> a

focal18_raw %>% 
  mutate(aggressor = sub(".*\\(", "",Event)) %>% 
  mutate(aggressor = sub(",.*","", aggressor)) %>% 
  mutate(victim = sub(".*,","",Event)) %>% 
  mutate(victim = sub("\\)","",victim)) -> a
