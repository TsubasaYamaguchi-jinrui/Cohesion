...
```{r}
## 発情メス
m_aggfocal_comp_est <- brm(no_agg ~ study_period + offset(logdur) + (1|subject),
                           family = negbinomial,
                           ...
                           file = "model/m_aggfocal_comp_est.rds")
```

**モデルチェック**  
```{r, include = FALSE}
check_aggfocal_comp_est <- dh_check_brms(m_aggfocal_comp_est)
```

過分散などの問題はないよう。  
```{r}
plotQQunif(check_aggfocal_comp_est)
```

ゼロ過剰の問題もない。  
```{r}
testZeroInflation(check_aggfocal_comp_est)
```

**結果の確認**  
2018年と2019年は2020年より有意に被攻撃頻度が高い。  
```{r}
estimate_contrasts(m_aggfocal_comp_est,
                   contrast = "study_period")
```

###### 非発情メス  
```{r}
m_aggfocal_comp_anest <- brm(no_agg ~ study_period + offset(logdur) + (1|subject),
                           family = negbinomial,
                           iter = 5000, warmup = 2500, seed = 13,
                           prior = c(prior(student_t(4,0,5), class = "b"),
                                     prior(student_t(4,0,10), class = "Intercept"),
                           ...
                           file = "model/m_aggfocal_comp_anest.rds")
```

**モデルチェック**  
```{r, include = FALSE}
check_aggfocal_comp_anest <- dh_check_brms(m_aggfocal_comp_anest)
```

過分散などの問題はない。  
```{r}
plotQQunif(check_aggfocal_comp_anest)
```

ゼロ過剰の問題もない。  
```{r}
testZeroInflation(check_aggfocal_comp_anest)
```

**結果の確認**  
2018年と2019年は2020年、2021年より有意に被攻撃頻度が高い。  
```{r}
estimate_contrasts(m_aggfocal_comp_anest,
                   contrast = "study_period")
```
...
**モデリング**  
```{r}
m_aggao_comp_est <- brm(no_agg ~ study_period + offset(logdur) + (1|femaleID),
                           family = negbinomial,
                           iter = 5000, warmup = 2500, seed = 13,
                           prior = c(prior(student_t(4,0,5), class = "b"),
                                     prior(student_t(4,0,10), class = "Intercept"),
                           ...
                           control=list(adapt_delta = 0.9999, max_treedepth = 20),
                           backend = "cmdstanr",
                           data =  agg_ao_all_fin %>% filter(rs2 == "1"),
                           file = "model/m_aggao_comp_est.rds")
```

**モデルチェック**  
```{r, include = FALSE}
check_aggao_comp_est <- dh_check_brms(m_aggao_comp_est)
```

過分散などの問題はないよう。  
```{r}
plotQQunif(check_aggao_comp_est)
```

ゼロ過剰の問題もない。  
```{r}
testZeroInflation(check_aggao_comp_est)
```

**結果の確認**  
2018年と2019年は2020年、2021年より有意に被攻撃頻度が高い。また、2019年は2018年よりも被攻撃頻度が高い。    
```{r}
estimate_contrasts(m_aggao_comp_est,
                   contrast = "study_period")
```

###### 非発情メス  
```{r}
m_aggao_comp_anest <- brm(no_agg ~ study_period + offset(logdur) + (1|femaleID),
                           family = negbinomial,
                           iter = 5000, warmup = 2500, seed = 13,
                           prior = c(prior(student_t(4,0,5), class = "b"),
                                     prior(student_t(4,0,10), class = "Intercept"),
                           ...
                           control=list(adapt_delta = 0.9999, max_treedepth = 20),
                           backend = "cmdstanr",
                           data =  agg_ao_all_fin %>% filter(rs2 == "0"),
                           file = "model/m_aggao_comp_anest.rds")
```

**モデルチェック**  
```{r, include = FALSE}
check_aggao_comp_anest <- dh_check_brms(m_aggao_comp_anest)
```

過分散などの問題はない。  
```{r}
plotQQunif(check_aggao_comp_anest)
```

ゼロ過剰の問題もない。  
```{r}
testZeroInflation(check_aggao_comp_anest)
```

**結果の確認**  
2019年 > 2018年 > 2021年 > 2020年。、  
```{r}
estimate_contrasts(m_aggao_comp_anest,
                   contrast = "study_period")
```
...
knitr::include_graphics("figure/p_aggao_study_period.png")
```
...
knitr::include_graphics("figure/p_aggao_study_period_2.png")
```
